  ...
  if (success) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6" style={{ background: '#0f0900' }}>
        <div className="max-w-md w-full text-center rounded-2xl p-10" style={{ background: '#140b00', border: '1px solid rgba(203,60,122,0.25)' }}>
          ...
          <Link to={createPageUrl('ProviderDashboard')}>
            <Button className="text-white border-0" style={{ background: '#cb3c7a' }}>Go to Dashboard</Button>
          </Link>
        </div>
      </div>
    );
            ...
            <React.Fragment key={s.id}>
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold"
                  style={{
                    background: step >= s.id ? '#cb3c7a' : 'rgba(255,255,255,0.1)',
                    color: step >= s.id ? '#fff' : 'rgba(255,255,255,0.4)'
                  }}>
                  {step > s.id ? <CheckCircle className="w-4 h-4" /> : s.id}
                </div>
                <span className="text-sm hidden sm:block" style={{ color: step === s.id ? '#cb3c7a' : 'rgba(255,255,255,0.45)' }}>
                  {s.label}
                </span>
              </div>
              {idx < STEPS.length - 1 && (
                <div className="flex-1 h-px" style={{ background: step > s.id ? '#cb3c7a' : 'rgba(255,255,255,0.1)' }} />
              )}
            </React.Fragment>
          ))}
        </div>
              ...
              <>
                ...
                <div className="flex justify-center mb-2">
                  <label className="cursor-pointer">
                    <div className="w-24 h-24 rounded-2xl border-2 border-dashed flex items-center justify-center overflow-hidden"
                      style={{ background: 'rgba(255,255,255,0.04)', borderColor: 'rgba(203,60,122,0.3)' }}>
                      {profileImage ? (
                        <img src={profileImage} alt="" className="w-full h-full object-cover" />
                      ) : (
                        <Camera className="w-8 h-8" style={{ color: '#cb3c7a' }} />
                      )}
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={formData.business_name} onChange={e => setFormData({...formData, business_name: e.target.value})}
                      ...
                      style={{ background: 'rgba(255,255,255,0.05)', borderColor: 'rgba(203,60,122,0.2)', color: '#fff' }}
                ...
                <div>
                  ...
                  <Select value={formData.category_id} onValueChange={v => setFormData({...formData, category_id: v})}>
                    <SelectTrigger style={{ background: 'rgba(255,255,255,0.05)', borderColor: 'rgba(203,60,122,0.2)', color: '#fff' }}>
                  ...
                  <div key={key}>
                    ...
                    <label className="cursor-pointer block">
                      <div className="p-4 rounded-xl border-dashed border-2 flex items-center gap-3 transition-colors"
                        style={{ borderColor: documents[key] ? '#10b981' : 'rgba(203,60,122,0.3)', background: documents[key] ? 'rgba(16,185,129,0.07)' : 'rgba(255,255,255,0.03)' }}>
                        {uploadingDoc === key ? (
                          <Loader2 className="w-6 h-6 animate-spin" style={{ color: '#cb3c7a' }} />
                        ) : documents[key] ? (
                          <CheckCircle className="w-6 h-6 text-green-400" />
                        ) : (
                          <Upload className="w-6 h-6" style={{ color: '#cb3c7a' }} />
                        )}
                        <div>
              ...
              <>
                <div>
                  ...
                  <div className="relative">
                    <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: '#cb3c7a' }} />
                  ...
                  <div className="grid grid-cols-4 md:grid-cols-7 gap-2">
                    {DAYS.map((day) => (
                      <button key={day} type="button" onClick={() => toggleDay(day)}
                        ...
                        style={{
                          background: selectedDays.includes(day) ? '#cb3c7a' : 'rgba(255,255,255,0.06)',
                          color: selectedDays.includes(day) ? '#fff' : 'rgba(255,255,255,0.6)',
                          borderColor: selectedDays.includes(day) ? '#cb3c7a' : 'rgba(255,255,255,0.1)'
                        }}>
                        {day.slice(0, 3)}
                      </button>
                    ))}
                ...
                <div className="p-4 rounded-xl" style={{ background: 'rgba(203,60,122,0.07)', border: '1px solid rgba(203,60,122,0.15)' }}>
