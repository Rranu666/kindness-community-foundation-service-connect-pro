      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl font-bold text-white"
            style={{ background: 'linear-gradient(135deg, #cb3c7a, #ef4444)' }}>
        ...
        <Tabs defaultValue="profile" className="w-full">
          <TabsList style={{ background: 'rgba(255,255,255,0.05)', borderColor: 'rgba(203,60,122,0.2)' }}
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1 p-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.05)' }}>
                    <Mail className="w-4 h-4" style={{ color: '#cb3c7a' }} />
                ...
                <div>
                  ...
                  {editMode ? (
                    ...
                    <div className="flex items-center gap-2 p-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.05)' }}>
                      <Phone className="w-4 h-4" style={{ color: '#cb3c7a' }} />
                ...
                {editMode && (
                  <div className="flex gap-3 pt-2">
                    <Button className="flex-1 text-white" style={{ background: '#cb3c7a' }}
          ...
          <TabsContent value="addresses" className="mt-0 space-y-4">
            {addressesLoading ? <Skeleton className="h-40" /> : addresses.map((address) => (
              <Card key={address.id} style={{ background: '#140b00', border: '1px solid rgba(203,60,122,0.2)' }}>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {address.is_default && (
                          <span className="text-xs px-2 py-0.5 rounded-full text-white" style={{ background: '#cb3c7a' }}>Default</span>
                        )}
                      </div>
            ...
            {savedProviders.length > 0 ? (
              <div className="space-y-3">
                {savedProviders.map((provider) => (
                  <Card key={provider.id} style={{ background: '#140b00', border: '1px solid rgba(203,60,122,0.2)' }}>
                    <CardContent className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex gap-2">
                          <Link to={createPageUrl(`ProviderProfile?id=${provider.id}`)}>
                            <Button size="sm" style={{ background: '#cb3c7a' }} className="text-white text-xs">View</Button>
                          </Link>
                          <Button size="sm" variant="ghost" className="text-red-400 hover:bg-red-400/10"
              ...
              <div className="text-center py-12">
                ...
                <Link to={createPageUrl('Browse')}>
                  <Button size="sm" style={{ background: '#cb3c7a' }} className="mt-4 text-white">Browse Providers</Button>
                </Link>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="reviews" className="mt-0">
            {reviews.length > 0 ? (
              <div className="space-y-3">
                {reviews.map((review) => (
                  <Card key={review.id} style={{ background: '#140b00', border: '1px solid rgba(203,60,122,0.2)' }}>
                    <CardContent className="p-4">
          ...
          <TabsContent value="settings" className="mt-0 space-y-4">
            {/* Language */}
            <Card style={{ background: '#140b00', border: '1px solid rgba(203,60,122,0.2)' }}>
              <CardHeader><CardTitle className="text-white flex items-center gap-2"><Globe className="w-5 h-5" style={{ color: '#cb3c7a' }} />Language & Region</CardTitle></CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {LANGUAGES.map((lang) => (
                    <button key={lang.code}
                      ...
                      style={{
                        background: selectedLang === lang.code ? 'rgba(203,60,122,0.15)' : 'rgba(255,255,255,0.04)',
                        borderColor: selectedLang === lang.code ? '#cb3c7a' : 'rgba(255,255,255,0.1)',
                        color: selectedLang === lang.code ? '#cb3c7a' : 'rgba(255,255,255,0.7)'
                      }}>
                      {lang.label}
            ...
            {/* Notifications */}
            <Card style={{ background: '#140b00', border: '1px solid rgba(203,60,122,0.2)' }}>
              <CardHeader><CardTitle className="text-white flex items-center gap-2"><Bell className="w-5 h-5" style={{ color: '#cb3c7a' }} />Notifications</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {[
            ...
            {/* Security */}
            <Card style={{ background: '#140b00', border: '1px solid rgba(203,60,122,0.2)' }}>
              <CardHeader><CardTitle className="text-white flex items-center gap-2"><Shield className="w-5 h-5" style={{ color: '#cb3c7a' }} />Account</CardTitle></CardHeader>
