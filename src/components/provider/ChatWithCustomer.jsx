...
export default function ChatWithCustomer({ open, onClose, order, senderEmail }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent style={{ background: '#140b00', borderColor: 'rgba(203,60,122,0.3)' }} className="border max-w-md">
        <DialogHeader>
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className="max-w-[80%] px-3 py-2 rounded-xl text-sm"
                  style={{
                    background: isMe ? '#cb3c7a' : 'rgba(255,255,255,0.1)',
                    color: '#fff',
        ...
        <div className="flex gap-2 pt-2 border-t border-white/10">
          ...
          <Button onClick={sendMessage} disabled={sending || !message.trim()}
            style={{ background: '#cb3c7a' }} className="text-white px-3">
