...
export default function ServiceMap({ providers, height = '400px' }) {
  if (!providers?.length) {
    return (
      <div style={{ height, background: '#140b00', border: '1px solid rgba(203,60,122,0.2)' }} className="rounded-lg flex items-center justify-center">
  ...
  return (
    <MapContainer
      ...
      zoom={12}
      style={{ height, borderRadius: '8px', border: '1px solid rgba(203,60,122,0.2)' }}
    >
      <TileLayer
          ...
          >
            <Popup>
              <div className="text-sm">
                ...
                <p className="text-xs font-semibold" style={{ color: '#cb3c7a' }}>⭐ {provider.rating}</p>
              </div>
            </Popup>
          </Marker>
        );
