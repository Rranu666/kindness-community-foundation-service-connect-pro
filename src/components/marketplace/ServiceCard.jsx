...
export default function ServiceCard({ service, onBook }) {
  return (
    <div
      ...
      style={{ background: '#13132a', border: '1px solid rgba(232,53,109,0.15)' }}
      onMouseEnter={e => e.currentTarget.style.borderColor = 'rgba(232,53,109,0.35)'}
      onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(232,53,109,0.15)'}
    >
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>
          ...
          <Badge variant="outline" className="text-xs" style={{ borderColor: 'rgba(232,53,109,0.25)', color: 'rgba(255,255,255,0.5)' }}>
        ...
        <div className="flex gap-2">
          {service.id && (
            <Link to={createPageUrl(`ServiceDetails?id=${service.id}`)}>
              <Button size="sm" variant="outline" className="text-white" style={{ borderColor: 'rgba(232,53,109,0.3)', background: 'transparent', height: 32 }}>
                Details
              </Button>
            </Link>
          )}
